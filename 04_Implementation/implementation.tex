...
\end{figure}
  ...
  Um der das Oberserver Pattern zu implementieren, wird ein Class \glqq Observable\grqq in diesem Projekt erstellt. (Abbildung ~\ref{fig:observable})
...
\end{figure}
  ...
  Vor den Zeichnen auf dem Whitebard werden 7 \glqq toggle box\grqq eingesetzt, dadurch die Funktion \glqq selectSection\grqq\ von HTC Vive Controller aufgerufen kann. (Abbildung ~\ref{fig:toggleBoxAbschnitte})
...
\end{figure}
  ...
  Wenn die Kollision zwischen der grüne Hinweis-Box und dem Objekt in Hand während des Loslösens auf den Hair-Trigger des Controllers detektiert wird, wird der entsprechende Zustand geändert und das Objekt auf den entsprechend Platz gelegt.
...
\end{table}
 ...
 Vier Geräusche werden bei der Kamera eingesetzt, und für den Druck und die Loslösung auf dem Hair-Trigger und dem Trackpad angepasst. Die built-in Funktion \glqq playSound\grqq\ wird in dem Objekt \glqq sound\grqq\ von \glqq componets\grqq\ eingepackt. Wenn ein Event von Controllers emittiert wird, wird die Funktion \glqq playSound\grqq\ des entsprechenden Sound Objekt aufgerufen. (Abbildung ~\ref{fig:submitSound})
...
\end{table}
 ...
 Um die Animation einfach zu benutzen, wird eine Funktion entwickelt. Das \glqq a-animation\grqq Element wird erstellt. Und nach dem Laufen der Animation wird das Element nach dem Bedarf, der als Parameter eingegeben, entworfen oder behaltet. (Abbildung ~\ref{fig:aAnimation})
